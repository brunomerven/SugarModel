...
* OBJ cost components
   %EQ%_OBJELS(REG,BD,CUR %SWD%) 	'Costs of elastic demands'
   %EQ%_OBJFIX(REG,CUR %SWD%) 		'Fixed Costs'
   %EQ%_OBJINV(REG,CUR %SWD%) 		'Investment component'
   %EQ%_OBJSALV(REG,CUR %SWD%) 		'Salvage'
   %EQ%_OBJVAR(REG,CUR %SWD%) 		'Variable operating costs'
   %EQ%_OBJDAM(REG,CUR %SWD%) 		'Damage costs'
...
*GG* V07_2 BLENDing equation
    EQL_BLND(R,ALLYEAR,BLE,SPE,ALLSOW) 		'Blending (=L=)'
    EQG_BLND(R,ALLYEAR,BLE,SPE,ALLSOW) 		'Blending (=G=)'
    EQE_BLND(R,ALLYEAR,BLE,SPE,ALLSOW) 		'Blending (=E=)'
    EQN_BLND(R,ALLYEAR,BLE,SPE,ALLSOW) 		'Blending non-binding'
