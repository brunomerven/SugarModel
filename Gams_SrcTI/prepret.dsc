...
$IFI '%RETIRE%'==YES $SHIFT KILL %TST%
  PRC_REFIT(RP,P)$0=0;
$IF NOT DEFINED %2 $EXIT
* Declarations
  SET VNRET(ALLYEAR,LL);
  PARAMETER RTFORC(R,LL,LL,P);
  PARAMETER RVPRL(R,LL,P);
* Interpolate RCAP_BND
$IF NOT DEFINED %TST% OPTION %TST% < %2;
  RCAP_BND(RTP,L)$0=0;
...
* Maximum salvage capacity
  %EQ%L_SCAP(RTP(R,V(LL),P),IPS%SOW%)$(((OBJ_SUMS(RTP)+(NOT PRC_VINT(R,P))$T(V))$RVPRL(RTP)$LIM(IPS) OR NCAP_OLIFE(RTP)$IO(IPS))$PRC_RCAP(R,P))..
   ...
   SUM(T(LL+RVPRL(RTP)),%VART%_SCAP(R,V,T,P%SWS%))$LIM(IPS)

   =L=  %VARV%_NCAP(R,V,P%SWS%)$T(V) + NCAP_PASTI(RTP) - %VAR%_SCAP(R,V,'0',P%SOW%)$OBJ_SUMS(RTP)$RVPRL(RTP);

* Retrofits and life-extensions
  %EQ%L_REFIT(RTP_TT(RT_PP(R,TT),T,PRC)%SWT%)..
   SUM((V(TT),P)$((VNT(T,V) OR PRC_REFIT(R,PRC,P)<0)$PRC_REFIT(R,PRC,P)),COEF_CPT(R,V,T,P)*(%VARV%_NCAP(R,V,P%SWS%)%RCAPSUB%)+%VART%_UDP(R,TT,T,PRC,'ANNUAL','N'%SWS%)$(ABS(PRC_REFIT(R,PRC,P))<1.5))
   =E=  
...
* Set bounds for continuous retirements
  ...
  %VAR%_RCAP.LO(RTP_CPTYR(R,V,T,P)%SOW%)$RVP(R,V,P) = RCAP_BND(R,T,P,'LO')$(NOT MAPVAL(RCAP_BLK(R,V,P)))+MIN(0,RTFORC(R,V,T,P)-RTFORC(R,V,TT,P)$Z);
  %VAR%_RCAP.UP(RTP_CPTYR(R,V,T,P)%SOW%)$RCAP_BND(R,T,P,'UP') =
...
* RESIDS require special handling
   SUM(RTP_CPTYR(R,PYR_S(V(K)),T,P)$((NOT RVPRL(R,V,P))$PRC_RCAP(R,P)), COEF_CPT(R,V,T,P) *
