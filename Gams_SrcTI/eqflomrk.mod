...
*---------------------------------------------------------------------------------------------------
 SET RTX_MARK(R,T,ITEM,C,BD,S) / EMPTY.EMPTY.EMPTY.EMPTY.EMPTY.EMPTY /;
 SET RX_MARK(R,ALLYEAR,ITEM,C,BD) / EMPTY.EMPTY.EMPTY.EMPTY.EMPTY /;
...
* Add ANNUAL level for PRC_MARK group commodities not in TOP
 LOOP((R,T,P,ITEM,C,BD)$PRC_MARK(R,T,P,ITEM,C,BD),TRACKC(R,C) = YES); RXX(TRACKC,'') = YES;
...
* Prepare the COMPRD variables for all PRC_MARK parameters:
 OPTION CLEAR=TRACKC; LOOP(RXX(R,C,''),TRACKC(RC(R,COM))$COM_GMAP(R,C,COM) = YES);
...
* If ITEM is also P (ala FLO_MARK), or C is not in TOP, use commodity timeslices; else use ANNUAL
 LOOP(RX_MARK(R,LL,P,C,BD),RXX(R,P,C)=YES);
 LOOP(RTX_MARK(R,T,ITEM,C,BD,S)$(NOT RC(R,C)),RXX(R,ITEM,C) = YES);
