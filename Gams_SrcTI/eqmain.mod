...
*-----------------------------------------------------------------------------
$   BATINCLUDE eqflobnd.%1 G LO "(FLO_BND(R,T,P,CG,S,'LO') NE 0)"
...
$IF NOT '%VAR_UC%' == YES $SETLOCAL UCBD ',BD' SETLOCAL UCLIM ',BD'
 ...
 SET UC_TMAP(ALLYEAR,ALLYEAR,T,SIDE,UC_DYNT);
 PARAMETER UC_SIGN(SIDE) / LHS 1, RHS -1 /;
...
LOOP((R,UC_N,T%UCLIM%)$UC_RHSRT(R,UC_N,T%UCLIM%),
 IF(    SUM(UC_TS_SUM(UC_R_EACH(R,UC_N),S),1)$UC_T_EACH(R,UC_N,T),UC_RHSMAP(UC_N,'EACH','SEVERAL',R,T,'ANNUAL'%UCBD%) = YES;
 ELSEIF SUM(UC_TS_SUM(UC_R_EACH(R,UC_N),S),1)$UC_T_SUCC(R,UC_N,T),UC_RHSMAP(UC_N,'SUCC','SEVERAL',R,T,'ANNUAL'%UCBD%) = YES;
));
LOOP((R,UC_N,T,S%UCLIM%)$UC_RHSRTS(R,UC_N,T,S%UCLIM%),
 IF(    (UC_R_EACH(R,UC_N)*UC_TS_EACH(R,UC_N,S))$UC_T_EACH(R,UC_N,T),UC_RHSMAP(UC_N,'EACH','EACH',R,T,S%UCBD%) = YES;
 ELSEIF (UC_R_EACH(R,UC_N)*UC_TS_EACH(R,UC_N,S))$UC_T_SUCC(R,UC_N,T),UC_RHSMAP(UC_N,'SUCC','EACH',R,T,S%UCBD%) = YES;
));
*-----------------------------------------------------------------------------
