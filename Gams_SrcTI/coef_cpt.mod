...
* capacity transfer
* [AL] Set here only if no alternate objective
  OPTION CLEAR=FIL;
$IF '%CTST%'==''  FIL(V) = YES;
  PASTSUM(RTP(R,FIL(V),P))$PRC_CAP(R,P) = B(V)+NCAP_ILED(R,V,P)+COEF_RPTI(R,V,P)*NCAP_TLIFE(R,V,P);
$IF '%VALIDATE%' == YES LOOP(PASTMILE(V)$(YEARVAL(V)=MIYR_V1-1),PASTSUM(R,V,P)$PASTSUM(R,V,P) = PASTSUM(R,V,P)+1);
