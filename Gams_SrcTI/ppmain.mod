...
* Additional Declarations in PPMAIN
  ...
  SET RTC_SHED(REG,ALLYEAR,COM,BD,J) 'Elastic shape indexes';
  SET RTP_CGC(REG,ALLYEAR,P,CG,CG);
  SET RTPCS_OUT(R,ALLYEAR,P,C,S);
  SET RTPS_BD(R,ALLYEAR,P,S,BD);
  SET UC_DYNDIR(REG,UC_N,SIDE) //;
  SET UC_DT(ALL_R,UC_N) //;
  SET RCS(REG,COM,TS);
  SET RC_RC(ALL_REG,COM,ALL_REG,COM);
  PARAMETER LEAD(ALLYEAR) / EMPTY 0 /;
  PARAMETER LAGT(ALLYEAR) / EMPTY 0 /;
  PARAMETER FPD(ALLYEAR) / EMPTY 0 /;
  PARAMETER IPD(ALLYEAR) / EMPTY 0 /;

*-----------------------------------------------------------------------------
* Establish label lengths for QC log (G225 and below)
$IF NOT SET G226 FILE NUL / NUL /; PUT NUL; NUL.LW=0; LOOP(ITEM, PUT @1 ITEM.TL; LABL(ITEM)=MAX(0,NUL.CC-13)); PUTCLOSE NUL;
*-----------------------------------------------------------------------------
...
* create list of all years in each period
    PERIODYR(T,EOHYEARS)$((YEARVAL(EOHYEARS) >= B(T)) *
                          (YEARVAL(EOHYEARS) <= E(T))) = YES;
*-----------------------------------------------------------------------------
...
$IF DEFINED G_OFFTHD $BATINCLUDE filparam G_OFFTHD '' '' ",'','','','','',''" ALLYEAR T '' '' 5
  G_OFFTHD(T)$NO = 0;
...
* EACHYEAR goes until (MIYR_VL+DUR_MAX)
    ...
    EACHYEAR(ALLYEAR)$((YEARVAL(ALLYEAR) >= MINYR) *
                       (YEARVAL(ALLYEAR) <= (MIYR_VL+DUR_MAX))) = YES;
...
*     else set the TS from TSL if non of them are provided
*-----------------------------------------------------------------------------
* remove invalid levels
    OPTION CLEAR=RXX; RXX(R,TSL,TSL)$(NOT SUM(RJLVL(J,R,TSL),1))=YES;
    F=CARD(COM_TSL); COM_TSL(R,C,TSL)$RXX(R,TSL,TSL)=NO; F=F-CARD(COM_TSL);
    Z=CARD(PRC_TSL); PRC_TSL(R,P,TSL)$RXX(R,TSL,TSL)=NO; Z=Z-CARD(PRC_TSL);
    IF(F+Z, PUTGRP=0;
$     BATINCLUDE pp_qaput.%1 PUTOUT PUTGRP 01 'Commodities/processes defined at non-existing TSLVL'
      PUT QLOG ' WARNING       - Number of COM/PRC resetted to ANNUAL level: ',F:0:0 '/' Z:0:0/);
...
*   - RPS_PRCTS corresponds to all the levels at/above PRC_TS
*   - RPS_S2 corresponds to the s2 term of EQ_PTRANS
*   - RPS_S1 corresponds to the s1 term of EQ_PTRANS

* Convert ANNUAL level timeslice storage
  RP_SGS(RP(R,P))$(PRC_TSL(R,P,'ANNUAL')$(PRC_MAP(R,'STS',P)+PRC_MAP(R,'NST',P))) = YES;
...
* For LOAD processes, take the maximum TSLVL
  ...
  PRC_YMAX(RP(R,P))$(NOT TRACKP(RP)) = SMAX((RPC_SPG(RP,C),COM_TSL(R,C,TSL)),TSLVLNUM(TSL));
  PRC_YMAX(TRACKP(R,P)) = SMAX((RPC(R,P,C),COM_TSL(R,C,TSLVL)),TSLVLNUM(TSLVL));
...
* save the finer of the PRC_TS and the finest commodity in the shadow primary
    PRC_YMAX(RP(R,P)) = MAX(PRC_YMAX(R,P),SMAX(PRC_TSL(R,P,TSLVL),TSLVLNUM(TSLVL)));
...
*-----------------------------------------------------------------------------
* clear infinite or zero CAP_BNDs
   OPTION CLEAR=RVP;
   LOOP(BDUPX(BD),RVP(RTP(R,T,P))$((CAP_BND(RTP,BD) EQ 0)$CAP_BND(RTP,BD)) = YES);
...
$IF NOT '%EXTEND%' == '' $BATINCLUDE main_ext.mod pp_prelv %EXTEND%
*-----------------------------------------------------------------------------
...
* CHP plants
    ...
    NCAP_CHPR(RTP(R,V,P),'UP')$((NOT SUM(BD$NCAP_CHPR(RTP,BD),1))$PRC_MAP(R,'CHP',P)) = 1;
...
* Get the timeslices where COM_FR has been given
    OPTION RCS < COM_FR; RCS(R,C,ANNUAL) = NO;
...
$IF NOT PARTYPE RTCS_FRMX  PARAMETER RTCS_FR(R,T,C,S,S) //;
    ...
    COM_FR(RTC(R,T,C),S)$((NOT COM_FR(RTC,S))$RCS_COMTS(R,C,S)) = G_YRFR(R,S);
...
* check sign of COM_ELAST and set RCJ after assigning default COM_STEPs
    ...
    OPTION RC_AGP < COM_ELAST; COM_STEP(RC_AGP(RC,BDNEQ))$(NOT COM_STEP(RC,BDNEQ)) = 1;
...
* commodities involved in CUM constraints
    LOOP(RC_CUMCOM(R,'NET',ALLYEAR,LL,C),
      RTC_CUMNET(R,T,C)$((E(T) >= YEARVAL(ALLYEAR)) * (B(T) <= YEARVAL(LL))) = YES;
$IFI %OBJ%==LIN  RTC_CUMNET(R,T,C)$((M(T)+LAGT(T) > YEARVAL(ALLYEAR)) * (M(T)-LEAD(T) < YEARVAL(LL))) = YES;
    );
    LOOP(RC_CUMCOM(R,'PRD',ALLYEAR,LL,C),
      RTC_CUMPRD(R,T,C)$((E(T) >= YEARVAL(ALLYEAR)) * (B(T) <= YEARVAL(LL))) = YES;
$IFI %OBJ%==LIN  RTC_CUMPRD(R,T,C)$((M(T)+LAGT(T) > YEARVAL(ALLYEAR)) * (M(T)-LEAD(T) < YEARVAL(LL))) = YES;
    );
* check all TS at/above COM_TSL for bounds
    RHS_COMBAL(RTCS_VARC(RTC_CUMNET(R,T,C),S)) = YES;
    ...
    RHS_COMPRD(RTCS_VARC(RTC_CUMPRD(R,T,C),S)) = YES;
...
* Levelization of STG_LOSS
  TRACKP(RP_STG(R,P))$(NOT RP_STS(R,P)) = YES;
...
* Convert equilibrium losses into standard losses for IPS
  STG_LOSS(RTP(R,V,P),ANNUAL(S))$((ABS(STG_LOSS(R,V,P,S)-0.5)>= 0.5)$PRC_MAP(R,'STK',P)) = 1-EXP(-ABS(STG_LOSS(R,V,P,S)));
  OPTION CLEAR=TRACKP;

*-----------------------------------------------------------------------------
* User constraints
*-----------------------------------------------------------------------------
* Assigning commodities and processes to the sets UC_GMAP_C and UC_GMAP_P
...
* Speed up code
 OPTION CLEAR=UNCD7;
 UNCD7('1',UC_N,SIDE,R,'ACT',T--ORD(T),P)  $= SUM(S$UC_ACT(UC_N,SIDE,R,T,P,S),1);
 UNCD7('2',UC_N,SIDE,R,'CAP',T--ORD(T),P)  $= UC_CAP(UC_N,SIDE,R,T,P);
 UNCD7('3',UC_N,SIDE,R,'NCAP',T--ORD(T),P) $= UC_NCAP(UC_N,SIDE,R,T,P);
 LOOP(UNCD7(J,UC_N,SIDE,R,UC_GRPTYPE,T,P), UC_GMAP_P(R,UC_N,UC_GRPTYPE,P) = YES);

* Mark those processes that have UC_CAP or COMXXX to also have VAR_CAP or VAR_COMXXX:
 LOOP(UC_GMAP_P(R,UC_N,'CAP',P), TRACKP(R,P) = YES);
 RTP_VARP(RTP(R,T,P))$TRACKP(R,P) = YES;
 OPTION CLEAR=TRACKP,CLEAR=RXX;
 LOOP(UC_GMAP_C(R,UC_N,COM_VAR,C,UC_GRPTYPE), RXX(R,COM_VAR,C) = YES);
...
* First, copy T_SUCC to T_EACH to reduce testing in what follows:
  UC_T_EACH(UC_T_SUCC) = YES; UC_DT(UC_R_EACH) = YES; UC_DT(UC_R_SUM) = YES;
...
* handle UC_FLO/ACT/COM leveling by aggregation/inheritance
...
RU_FEQ(R,COM,T)$(NOT RU_FEQ(R,COM,T)) = 1;
