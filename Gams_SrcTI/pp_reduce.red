...
* Set for FLO_FUNC/FLO_SUM based substitution
 SET RPCG_PTRAN(R,P,COM,COM,CG,CG) / EMPTY.EMPTY.EMPTY.EMPTY.EMPTY.EMPTY /;
...
PRC_CAP(RP(R,P))     = YES;
PRC_ACT(RP(R,P))     = YES;
PRC_ACT(R,P)$PRC_MAP(R,'STG',P) = NO;
...
* processes without capacity variable
NO_CAP(RP(R,P))$(NOT PRC_CAP(R,P)) = YES;
...
TRACKPC(RP_STD(R,P),C)$(TOP(R,P,C,'OUT')*COM_TMAP(R,'ENV',C)) = YES;
...
*--------------------------------------------------------------
  RPC_ACT(RPC_PG(RP_PGACT(RP_STD(R,P)),C)) = YES;

* VAR_IRE can be replaced by VAR_ACT if only 1 import or export
* commodity (might not be true when exchange process has efficiency???)

  RPC_AIRE(RPC_PG(RP_PGACT(RP_IRE(R,P)),C)) = YES;

* If activity is substituted for the primary flow, EQ_ACTFLO is not needed
  PRC_ACT(RP_PGACT(R,P)) = NO;
*--------------------------------------------------------------------------------------------
* Process without capacity variable and without activity related parameters
* does not need activity variable
*--------------------------------------------------------------------------------------------
 NO_ACT(PRC_ACT(NO_CAP(R,P))) = YES;
...
* Remove activity flows and emission flows from RPC_FFUNC
 RPC_FFUNC(RPC(R,P,C))$(RPC_ACT(R,P,C)+RPC_EMIS(R,P,C)) = NO;
...
*--------------------------------------------------------------------------------------------
* [AL] Speed-up & correction (RTP_VARA should NOT be deleted if only some PRC_TS is fixed to zero)
...
LOOP((RTP(R,T,P),RPCS_VAR(R,P,C,S))$RTPCS_OUT(R,T,P,C,S),
...
* Track commodity which are turned off by some process: by T, timeslice and IO:
  LOOP(RTPS_OFF(R,T,P,S),IF(RP_IRE(R,P),RTCS_SING(R,T,C,S,'OUT')$RPC_IRE(R,P,C,'IMP') = YES;
                                   ELSE RTCS_SING(R,T,C,S,IO)$TOP(R,P,C,IO) = YES));
* Track commodities which are turned off by some process: by IO only:
  ...
  RXX(TRACKC,'OUT') = NO; OPTION CLEAR=TRACKC;
...
* turning off all processes in linking to RTCS_SING
  LOOP(RTCS_SING(R,T,C,S,'OUT'),
    LOOP(TOP(R,P,C,'IN')$(SUM(COM$TOP(R,P,COM,'IN'),1) EQ 1), RTPS_OFF(R,T,P,S)$RPS_PRCTS(R,P,S) = YES;));
  LOOP(RTCS_SING(R,T,C,S,'IN'),
    LOOP(TOP(R,P,C,'OUT')$(SUM(COM$TOP(R,P,COM,'OUT'),1) EQ 1), RTPS_OFF(R,T,P,S)$RPS_PRCTS(R,P,S) = YES;));
