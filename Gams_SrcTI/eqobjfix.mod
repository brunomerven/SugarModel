...
*  - the test for relevant costs is done on the year of installation, but perhaps
*    should be Y-running year
...
* Set up the rounded lifetimes; round half years down:
 OBJ_DIVIV(OBJ_YES(R,V,P)) = MAX(1+1$PRC_RESID(R,'0',P)$PYR_S(V),MIN(CARD(AGE),CEIL(NCAP_TLIFE(R,V,P)-0.5)));
*======================================================================================
* Apply SHAPE to OBJ_YES:
*--------------------------------------------------------------------------------------
OPTION CLEAR=RTP_SHAPE;
LOOP(SAMEAS(J,'1'),
 RTP_SHAPE(OBJ_YES(R,V,P),'1',J+MAX(0,NCAP_FOMX(R,V,P)-1),J+MAX(0,NCAP_FOMM(R,V,P)-1))$(NCAP_FOMX(R,V,P)+NCAP_FOMM(R,V,P)) = YES;
 RTP_SHAPE(OBJ_YES(R,V,P),'2',J+MAX(0,NCAP_FTAXX(R,V,P)-1),J+MAX(0,NCAP_FTAXM(R,V,P)-1))$(NCAP_FTAXX(R,V,P)+NCAP_FTAXM(R,V,P)) = YES;
 RTP_SHAPE(OBJ_YES(R,V,P),'3',J+MAX(0,NCAP_FSUBX(R,V,P)-1),J+MAX(0,NCAP_FSUBM(R,V,P)-1))$(NCAP_FSUBX(R,V,P)+NCAP_FSUBM(R,V,P)) = YES;
);
 RTP_SHAPE(RTP_SHAPE(R,V,P,J,'1','1')) = NO;
$IF NOT '%VALIDATE%' == YES  RTP_ISHPR(R,V,P)$(NCAP_FOMX(R,V,P)+NCAP_FTAXX(R,V,P)+NCAP_FSUBX(R,V,P)) = YES;
$IF NOT '%VALIDATE%' == YES  RTP_ISHPR(R,V,P)$(NCAP_FOMM(R,V,P)+NCAP_FTAXM(R,V,P)+NCAP_FSUBM(R,V,P)) = YES;
$IF '%VALIDATE%' == YES      RTP_ISHPR(RTP(R,V,P)) = YES;
*=====================================================================================
