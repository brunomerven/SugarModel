...
*-----------------------------------------------------------------------------
* Version control
$IF NOT FUNTYPE rpower $ABORT TIMES Version 3.8.2 and above Requires GAMS 22.0 or above!
...
* demands
  SET DEM_SMAP(REG,DEM_SECT,COM)     'Grouping of DMs (commodities) to their sector' //;
...
* region
*  SET REG(ALL_REG)                   'List of Regions'           / EMPTY /;
...
* user constraints
  SET UC_N                            'Names of all manual constraints'      / EMPTY /;
  ...
  SET UC_R_SUM(ALL_REG,UC_N)          'Specification of regions, if UC_REG=SUM'     //;
  SET UC_R_EACH(ALL_REG,UC_N)         'Specification of regions, if UC_REG=EACH'    //;
  ...
  SET UC_TSL(R,UC_N,SIDE,TSLVL)       'UC timeslice level' //;
  ALIAS(UC_N,UCN);
...
* New Capacity
  ...
  PARAMETER NCAP_CHPR(REG,ALLYEAR,PRC,BD)       'Combined heat:power ratio'               //;
...
* demands
  ...
  PARAMETER COM_ELAST(REG,ALLYEAR,COM,TS,LIM)       'Easticity of demand'               //;
  PARAMETER COM_ELASTX(REG,ALLYEAR,COM,BD)          'Easticity shape of demand'         //;
  PARAMETER COM_PROJ(REG,ALLYEAR,COM)               'Demand baseline projection'        //;
  PARAMETER COM_STEP(REG,COM,BD)                    'Step size for elastic demand'      //;
...
* trade of commodities
  PARAMETERS
    ...
    IRE_FLO(ALL_R,ALLYEAR,PRC,COM,ALL_R,COM,TS)     'Efficiency of exchange for inter-regional transder' //
    IRE_FLOSUM(REG,ALLYEAR,PRC,COM,TS,IE,COM,IO)    'Aux. consumption/emissions from inter-regional transder' //
...
* process
  ...
  SET RPS_S1(R,P,ALL_TS)        'All timeslices at the PRC_TSL/COM_TSLspg = PTRANSs1' //;
  SET RPS_S2(R,P,ALL_TS)        'All timeslices at/above the PRC_TSL/COM_TSLspg = PTRANSs2' //;
  ...
  SET RTP_TT(R,T,T,PRC)         'Retrofit control periods'               //;
  SET RVP(R,ALLYEAR,P)          'ALIAS(RTP) for Process/time'            //;
  SET RTP_CAPYR(R,ALLYEAR,ALLYEAR,P) //;
  SET CG_GRP(REG,PRC,CG,CG)          //;
  SET FSCK(REG,PRC,CG,C,CG)          //;
  SET FSCKS(REG,PRC,CG,C,CG,S)       //;
  SET RPC_IREIO(R,P,C,IE,IO)         //;
  SET RD_AGG(REG,COM)                //;
...
* internal Parameters
*         PARAMETER ANNC_BLE(BLE,OPR,YEAR)       //;
*         PARAMETER PRICE_BLE(YEAR,BLE,OPR)      //;
*         PARAMETER BAL_BLE(YEAR,COM,COM)        //;
...
*-----------------------------------------------------------------------------
  ...
  SCALAR     MY_F /0/, F /0/, Z /0/, CNT /0/, DFUNC /0/, DONE /0/;
...
*------------------------------------------------------------------------------
* One predefined system CG and COM: ACT
...
* Internal Sets:
 SETS
   ...
   R_UC(R,UC_N)          'Temporary set for UCs by region' //
...
*------------------------------------------------------------------------------
SET    NO_CAP(R,P)                    'Process not requiring capacity variable'             //;
...
*------------------------------------------------------------------------------
  PARAMETERS
   ...
   PAR_OBJFLO(R,ALLYEAR,ALLYEAR,P,COM,TS,CUR)  'Annual discounted flow costs'                           //
   PAR_OBJCOM(R,ALLYEAR,COM,TS,CUR)            'Annual discounted commodity costs (incl import/export)' //
