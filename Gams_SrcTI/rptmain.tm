...
*---------------------------------------------------------------
*  - creates VEDA file %RUN_NAME%.vd and %RUN_NAME%.obj for annual objective values
...
*  - output of annual cost terms in *.vd file
*
*-----------------------------------------------------------------------
...
* calculate discounting for the period
  PARAMETER VDA_DISC(R,ALLYEAR) / EMPTY.EMPTY 0 /;
* [UR]: Calculation of shadow prices in MACRO
  ...
  NCAP_YES(RTP)$(VAR_NCAP.L(RTP)+NCAP_PASTI(RTP) GT 0) = YES;
...
*------------------------------------------------------------------------
PAR_FLO(RTP_VINTYR(R,V,T,P),C,S)$(RTPCS_VARF(R,T,P,C,S)*RP_FLO(R,P)*(NOT RPC_CONLY(R,T,P,C))) $=
                     EPS$(NOT PAR_FLO(R,V,T,P,C,S));
          ;
PAR_IRE(RTP_VINTYR(R,V,T,P),C,TS,IMPEXP)$(RTPCS_VARF(R,T,P,C,TS)$RPC_IRE(R,P,C,IMPEXP)) $=
                     EPS$(NOT PAR_IRE(R,V,T,P,C,TS,IMPEXP));
          ;
*---------------------------------------------------------------------
* Discounted objective values by cost type(INV, FIX, VAR etc.)
* - output in file %RUN_NAME%.obj and %RUN_NAME%.vd
*---------------------------------------------------------------------

* Discounted objective value by region
PARAMETER REG_OBJ(REG) /EMPTY 0/;
...
PARAMETER PAR_Y(R,T)	   'Production parameter'             /EMPTY.EMPTY 0/;
PARAMETER PAR_GRY(R,T)	   'Growth rate of production'        /EMPTY.EMPTY 0/;
PARAMETER PAR_GRGDP(R,T)   'Growth rate of GDP'               /EMPTY.EMPTY 0/;
PARAMETER PAR_MC(R,LL,C)   'Marginal cost demand commodity'   /EMPTY.EMPTY.EMPTY 0/;
PARAMETER TM_GDP(R,T)	   'Current GDP parameter'            /EMPTY.EMPTY 0/;
PARAMETER TM_GDPGOAL(R,LL) 'Baseline GDP';
PARAMETER TM_DEM(R,ALLYEAR,C);
