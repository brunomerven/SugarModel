...
* Annualizing coefficient for investment costs over years of capacity transfer
  COEF_CRF(OBJ_ICUR(R,V,P,CUR)) = (1-(1/(1+G_DRATE(R,V,CUR)))) / (1-(1+G_DRATE(R,V,CUR))**(-(NCAP_TLIFE(R,V,P)*COEF_RPTI(R,V,P))));

* Investment cost coefficient

  COEF_OBINV%1(OBJ_ICUR(R,V,P,CUR)) = SUM(OBJ_SUMSI(R,V,P,K),COEF_CRF(R,V,P,CUR)/OBJ_DISC(R,K,CUR)) * (
