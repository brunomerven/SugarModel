...
*$ONLISTING
*
    %EQ%_PEAK(%R_T%,CG2,SL %SWT%)$(SUM(COM_GMAP(R,CG2,C)$RTC(R,T,C),1)$COM_PKTS(R,CG2,SL)) ..
...
*GG*PK no multiplier
*[UR] 25.04.2003 added NCAP_PKCNT multiplier to turn off flow variables by setting NCAP_PKCNT to zero
*[AL] 25.01.2007 allow using PKNO to allocate process to production-based peak contribution
