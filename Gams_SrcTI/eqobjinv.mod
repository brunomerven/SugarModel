...
* Operating years for all technical lifetimes
 SET OPYEAR(AGE,LIFE) //;
 MAXLIFE = MAX(SMAX(T,2*D(T)),CEIL(MAXLIFE));
...
*===============================================================================
  LOOP(OBJ_SUMII(R,V,P,LIFE,K,JOT), KAGE(K,JOT) = YES);
  INVSTEP(LL,JOT,LL+(ORD(AGE)-1),AGE+(ORD(JOT)-ORD(AGE)))$(KAGE(LL,JOT)*OPYEAR(JOT,AGE)) = YES;
...
* PASTI charge
    SUM(OBJ_SUMII(R,PASTYEAR(V),P,LIFE,K_EOH,JOT)$OBJ_PASTI(R,V,P,CUR),
...
*------------------------------------------------------------------------------
  ...
  OPTION CLEAR = OBJ_YES;
  OPTION CLEAR = OBJ_SUMS3;
  OPTION CLEAR = OBJ_I2;
  OPTION CLEAR = OBJ_SPRED;
